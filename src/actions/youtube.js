...
export function searchVideos(config) {
  return async (dispatch) => {
    try {
      dispatch({ type: 'SET_QUERY', data: { query: config.query } });
...
export function getFeed() {
  return async (dispatch) => {
    try {
      ...
      const getItems = async (pageToken = '') => {
        const { items: newItems, nextPageToken } = await api.getSubscriptions({
          mine: true,
          pageToken
        });
      ...
      const feeds = await Promise.all(
        channelIds.map(({ channelId }) =>
          parseFeed(`${PROXY_URL}/${FEED_URL}?channel_id=${channelId}`)
        )
      );
      ...
      const data = feeds
        .reduce((arr, { items }) => arr.concat(items.map(parseVideo)), [])
        .sort(sortDesc('publishedAt', (date) => new Date(date).getTime()));
